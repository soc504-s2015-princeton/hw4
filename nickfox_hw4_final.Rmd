...
data <- within(data, location <- relevel(location, ref = "other")) #relevel factors
data <- within(data, language <- relevel(language, ref = "French")) #relevel factors

mylogit2 <- glm(decision ~ judge + rater + location + language, data = data, family = "binomial")
...
```{r}
prob_Marceau2 <- 1 / (1 + exp(-(estimates2[1] + estimates2[7] * 1 + estimates2[11] * 1 + estimates2[13] * 1 + estimates2[14] * 1)))

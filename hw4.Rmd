...
## load data
data <- read.table("data/Greene.txt") # <- could not load with readr library. data has irregular spacing.

## set data as dplyr table
data <- tbl_df(data)
...
```{r}
##calcualte the proportion of yes and no decisions by judge
appeals <- data %>%
  group_by(decision) %>%
  tally() %>%
  mutate(prop = n / sum(n))
...
#[Appeals were granted in ~ 30% of the cases]
...
## claculate the number of cases per judge
judges <- data %>%
  group_by(judge) %>%
  tally() %>%
  arrange(desc(n)) %>%
  mutate(judge = factor(judge, judge)) # change levels so that plot keeps arrange by n

ggplot(judges, aes(judge, n)) + geom_histogram(stat = "identity") + ylab("Cases")
...
```{r}
##calculate porpotion of appeals granted by judge
judges_prop <- data %>%
  group_by(judge, decision) %>%
  tally() %>%
  mutate(prop = n / sum(n)) %>%
  filter(decision == "yes") %>% # only keep those granted
  ungroup() %>%
  arrange(desc(prop)) %>%
  mutate(judge = factor(judge, judge)) # change levels to keep arrange by prop of appeals
  

ggplot(judges_prop, aes(judge, prop)) + geom_histogram(stat = "identity") +
  ylab("Proportion")
...
##relevel judge factor to have Iacobucci as reference
data$judge <- relevel(data$judge, ref = "Iacobucci")

##run logistic regression model
m1 <- glm(decision ~ judge, data = data, family = binomial)
...
## create new data frame to set Marceu fixed
data_marceau <- data
## set marceu fixed
data_marceau$judge <- "Marceau"

## use predict with Marceau fixed
pp_marceau <- predict(m1, type = "response", newdata = data_marceau)

##calculate mean of predicted values to get predicted probability of observed cases
mean_marceau <- mean(pp_marceau)
mean_marceau

#[the predicted probabilty that Marceu will grant an appeal is 0.6]
...
## relevel location to have "other" as reference
data$location <- relevel(data$location, ref = "other")

##relevel language and ser French as reference
data$language <- relevel(data$language, ref = "French")

## run logistic regression model
m2 <- glm(decision ~ judge + rater + location + language, data = data, family = binomial)
...
data_ytem$rater <- "yes"
data_ytem$location <- "Toronto"
data_ytem$language <- "English"
data_ytem$judge <- "Marceau"

pp_ytem <- predict(m2, type = "response", newdata = data_ytem)

##calculate mean of predicted values to get predicted probability of observed cases
mean_ytem <- mean(pp_ytem)
mean_ytem

#[the predicted probabilty that Marceu will grant an appeal that had merit, took place in Toronto and that was in english is ~ 0.9]

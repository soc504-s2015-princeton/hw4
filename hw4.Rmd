...
## load data
data <- read.table("data/Greene.txt") # <- could not load with readr library.
                                        #  data has irregular spacing.
...
judges <- data %>%
  ...
  tally() %>%
  arrange(desc(n)) %>%
  mutate(judge = factor(judge, judge)) # change levels so that 
                                       # plot keeps arrange by n

ggplot(judges, aes(judge, n)) +
...
judges_prop <- data %>%
  ...
  ungroup() %>%
  arrange(desc(prop)) %>%
  mutate(judge = factor(judge, judge)) # change levels
                                        # to keep arrange by prop of appeals
